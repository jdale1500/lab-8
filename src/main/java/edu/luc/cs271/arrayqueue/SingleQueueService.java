...
public class SingleQueueService {
  ...
  public static void main(final String[] args) throws InterruptedException {
    // Finished read successive input lines until EOF and try to add them to the queue
    ...
    final Thread consumer =
		...
		int remaining;
                synchronized (lock) {
                  current = queue.poll(); // Finished try to take next name from queue
		  remaining = queue.size(); // Finished determine resulting size of queue
                }
                if (current == null) {
    ...
    while (input.hasNextLine()) {
      ...
      boolean result;
      synchronized (lock) {
        result = queue.offer(name); // Finished try to add this name tothe queue
      }
      if (result) {
